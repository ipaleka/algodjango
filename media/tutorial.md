...
The code for `_wait_for_confirmation` function is still our obligation for version `1.5.0` of the `py-algorand-sdk`, but it will probably be a part of Algorand's Python SDK in the future.
...
Let's do one more thing before we move to the next section. If we now visit the project's root/index page then both the standalone and wallet-based accounts will show up. Update the index view and use the Django object-relational mapper to exclude the wallet-based accounts:
